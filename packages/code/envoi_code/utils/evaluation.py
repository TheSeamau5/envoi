...
def _build_evaluation_python_script(
    *,
    ...
    return (
        ...
        "def _load_environment_test_sources():\n"
        "    mapping = {}\n"
        ...
        "                file_source = _read_text(test_file)\n"
        ...
        "            if not source:\n"
        "                continue\n"
        "            normalized = _normalize_suite_path(test_path)\n"
        "            if normalized:\n"
        ...
        "        main_source = _read_text(environment_main)\n"
        ...
        "        except Exception:\n"
        "            pass\n"
        "def _suite_source_candidates(suite):\n"
        "    normalized = _normalize_suite_path(suite)\n"
        "    if not normalized:\n"
        "        return []\n"
        ...
        "    return deduped\n"
        "def _attach_test_sources(tests, source_map):\n"
        "    if not isinstance(source_map, dict) or not source_map:\n"
        "        return tests\n"
        "    for test in tests:\n"
        "        if not isinstance(test, dict):\n"
        "            continue\n"
        "        current_source = _as_str(test.get('source'))\n"
        "        if current_source and current_source.strip():\n"
        "            continue\n"
        "        suite = _as_str(test.get('suite')) or ''\n"
        "        resolved_source = None\n"
        "        for candidate in _suite_source_candidates(suite):\n"
        "            value = source_map.get(candidate)\n"
        "            if isinstance(value, str) and value.strip():\n"
        "                resolved_source = value.strip()\n"
        "                break\n"
        "        if resolved_source is None:\n"
        "            normalized_suite = _normalize_suite_path(suite)\n"
        ...
        "        source = _as_str(case.get('c_source') or case.get('source'))\n"
        "        out.append({\n"
        "            'suite': suite_name,\n"
        "            'test_id': test_id,\n"
        "            'status': status,\n"
        ...
        "            duration_ms = None\n"
        "        suite_name = _normalize_suite_path(suite)\n"
        "        source = _as_str(item.get('source') or item.get('c_source'))\n"
        "        out.append({\n"
        "            'suite': suite_name,\n"
        "            'test_id': test_id,\n"
        "            'status': status,\n"
        ...
        "    ]\n"
        "    test_source_map = _load_environment_test_sources()\n"
        "    payload = {\n"
        "        'duration_ms': 0,\n"
        "        'passed': 0,\n"
        "        'failed': 0,\n"
        ...
        "                    extracted_tests = _extract_tests(result, suite_key)\n"
        "                    extracted_tests = _attach_test_sources(\n"
        "                        extracted_tests,\n"
        "                        test_source_map,\n"
        "                    )\n"
        "                    extracted_tests = _dedupe_tests(extracted_tests)\n"
