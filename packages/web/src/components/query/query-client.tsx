...
export function QueryClient({
  ...
  }, [saveTemplateName, setSavedTemplates, sql, savedTemplates]);

  const handleDeleteSavedTemplate = useCallback(
    (templateId: string) => {
      setSavedTemplates(
        savedTemplates.filter((tmpl) => tmpl.id !== templateId),
      );
      if (activeTemplateId === templateId) {
        setActiveTemplateId(undefined);
      }
    },
    [activeTemplateId, savedTemplates, setSavedTemplates],
  );
  ...
  return (
    <div className="flex flex-1 overflow-hidden">
      ...
      <div className="flex w-65 shrink-0 flex-col overflow-y-auto border-r border-envoi-border bg-envoi-surface">
        <div className="flex items-center justify-between border-b border-envoi-border px-3 py-1.5">
          <span className="text-[13px] font-semibold uppercase tracking-[0.08em] text-envoi-text-dim">
            Templates
          </span>
          <button
            onClick={() => setShowSaveForm((prev) => !prev)}
            className="flex items-center gap-1 rounded px-1.5 py-0.5 text-[13px] text-envoi-text-muted transition-colors hover:bg-envoi-border-light hover:text-envoi-text"
        ...
        {/* Save form */}
        {showSaveForm && (
          <div className="flex flex-col gap-1.5 border-b border-envoi-border px-3 py-2">
            <input
              type="text"
              value={saveTemplateName}
              onChange={(event) => setSaveTemplateName(event.target.value)}
              placeholder="Template name..."
              className="rounded border border-envoi-border-light bg-envoi-bg px-2 py-1 text-[12px] text-envoi-text outline-none focus:border-envoi-accent"
            />
            <button
              onClick={handleSaveTemplate}
              disabled={
                saveTemplateName.trim().length === 0 || sql.trim().length === 0
              }
              className="flex items-center justify-center gap-1 rounded bg-envoi-accent px-2 py-1 text-[13px] font-semibold text-white transition-opacity disabled:opacity-40"
            >
        ...
        {/* Built-in templates */}
        <div className="border-b border-envoi-border px-3 py-1">
        ...
        {builtinTemplates.map((tmpl) => (
          <button
            ...
            className={`border-b border-envoi-border-light px-3 py-2 text-left transition-colors ${
          ...
          >
            ...
            <div className="mt-0.5 text-[13px] text-envoi-text-dim">
              {tmpl.description}
            </div>
          </button>
        ))}
            ...
            {savedTemplates.map((tmpl) => (
              ...
              >
                ...
                <button
                  ...
                  className="shrink-0 px-2 py-2.5 text-envoi-text-dim transition-colors hover:text-envoi-red"
        ...
        {builtinTemplates.length === 0 && savedTemplates.length === 0 && (
          <div className="px-3 py-2.5 text-[12px] text-envoi-text-dim">
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        {activeTemplate && activeTemplate.parameters.length > 0 && (
          <div className="flex items-end gap-2.5 border-b border-envoi-border px-3.5 py-2">
            ...
            <button
              ...
              className="rounded px-2 py-1.25 text-[13px] text-envoi-text-muted transition-colors hover:bg-envoi-border-light hover:text-envoi-text"
            >
              Clear
            </button>
          </div>
        ...
        {/* SQL Editor */}
        <div className="flex flex-col gap-2 border-b border-envoi-border px-3.5 py-2.5">
          <textarea
            ref={textareaRef}
            ...
            className="min-h-30 w-full resize-y rounded border border-envoi-border-light bg-envoi-surface px-2.5 py-2 text-[12px] leading-5 text-envoi-text outline-none focus:border-envoi-accent"
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 rounded bg-envoi-accent px-3 py-1.25 text-[13px] font-semibold text-white transition-opacity disabled:opacity-40"
            >
              {isLoading ? (
            ...
            <span className="text-[13px] text-envoi-text-dim">Cmd+Enter</span>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-auto">
          ...
          {result && (
            <div className="flex flex-col">
              {/* Stats bar */}
              <div className="flex items-center gap-2.5 border-b border-envoi-border-light px-3.5 py-1">
                    ...
                    <thead>
                      <tr>
                        {result.columns.map((col) => (
                          <th
                            key={col}
                            className="sticky top-0 border-b border-envoi-border bg-envoi-surface px-2.5 py-1 text-left font-semibold text-envoi-text-muted"
                          >
                            {col}
                          </th>
                        ))}
                      ...
                      {result.rows.map((row, rowIndex) => (
                        ...
                        >
                          {result.columns.map((col) => (
                            <td
                              key={col}
                              className="border-b border-envoi-border-light px-2.5 py-0.75 text-envoi-text"
                              style={{
                                maxWidth: 300,
                                overflow: "hidden",
      ...
      <div className="flex w-60 shrink-0 flex-col overflow-y-auto border-l border-envoi-border bg-envoi-surface">
        <div className="border-b border-envoi-border px-3 py-1.5">
          <span className="text-[13px] font-semibold uppercase tracking-[0.08em] text-envoi-text-dim">
            Schema
          </span>
        </div>
  ...
  return (
    <div className="border-b border-envoi-border-light">
      <button
        ...
        className="flex w-full items-center gap-1.5 px-3 py-1.5 text-left text-[12px] font-semibold text-envoi-text hover:bg-envoi-border-light"
      >
      ...
      {expanded && (
        <div className="pb-1">
          {columns.map((col) => (
            <div
              key={col.columnName}
              className="flex items-center gap-1.5 px-6 py-0.5 text-[13px]"
            >
...
function BarChart({ result }: { result: QueryResult }) {
  ...
  const valueCol =
    result.columns.length > 2 ? result.columns[2] : result.columns[1];
    ...
    <div className="mb-3.5">
      ...
      >
        ...
        {data.map((point, barIndex) => {
          const barX =
            BAR_MARGIN.left +
            (barIndex / data.length) * BAR_PLOT_W +
            (BAR_PLOT_W / data.length - barWidth) / 2;
...
function LineChart({ result }: { result: QueryResult }) {
  ...
  const toX = (value: number) =>
    LINE_MARGIN.left + ((value - xMin) / xRange) * LINE_PLOT_W;
  const toY = (value: number) =>
    LINE_MARGIN.top + LINE_PLOT_H - (value / yMax) * LINE_PLOT_H;

  const pathD = data
    .map(
      (point, pointIndex) =>
        `${pointIndex === 0 ? "M" : "L"}${toX(point.x).toFixed(1)},${toY(point.y).toFixed(1)}`,
    )
    .join(" ");
  ...
  return (
    <div className="mb-3.5">
      ...
      >
        ...
        <text
          x={LINE_MARGIN.left - 8}
          y={LINE_MARGIN.top + LINE_PLOT_H + 3}
          textAnchor="end"
          style={{ fontSize: "10px", fill: T.textDim }}
        >
          0
